...
class DFLogger:
    ...
    def __init__(self, column_names, logger_name='df_logger', sep='|', log_filename='log.txt', log_dir_suffix='',
                 linewidth=3, plot_font_size=18, plot_style='tableau-colorblind10', figure_size=(10, 8),
                 copy_config_files=True, files_to_copy_start_with=['1_', '2_'], plot_ungrouped=['lr', 'epoch'],
                 figure_ext='.png'):
        self.column_names = []
        self.SEP = sep
        ...
        self.plot_style = available_styles[0] if plot_style not in available_styles else plot_style

        self.parse_column_names(column_names)

        self.log_filename = log_filename
        self.log_dir_suffix = log_dir_suffix
        self.log_dir = os.path.join('./logs', datetime.now().strftime(f"%Y-%m-%d_%H-%M-%S-{self.log_dir_suffix}"))
    ...
    def save_val_predictions(self, df, f1=False):
        ...
        df.to_csv(os.path.join(self.plots_directory, 'best_val_predictions'+filesuffix), index=False)

        filesuffix = '_f1.pkl' if f1 else '_loss.pkl'
        df.to_pickle(os.path.join(self.plots_directory, 'best_val_predictions'+filesuffix))
    ...
    def log_values_list(self, values, screen_print=False):
        assert len(values) == self.n, "Logging a list of values which is not the same size as the number of columns of the logger"
        assert self.current_index == 0, "Logging a line while the pointer is not at the start of a line"
    ...
    def read_current_log_file(self):
        return pd.read_csv(self.log_filepath, sep=self.SEP)

    def plot_columns(self, flag_column='flag', batches_per_epoch=0, epoch=0, log_interval=0):
        df = self.read_current_log_file()
        group = True

        if flag_column not in df.columns:
            print("Flag column name {} is not one of the columns of the logged df file."
                  "Plotting without grouping.", flag_column)
            group = False
        ...
        for c in df.columns:
            ...
            plot_filepath = os.path.join(self.plots_directory, clean_c+self.figure_ext)

            try:
                if group and c not in self.plot_ungrouped:
                    df.groupby(flag_column)[c].plot(title=c, legend=True, linewidth=self.linewidth)
                else:
                    if flag_column in df.columns:
                        df[df[flag_column] == 'train'][c].plot(title=c, legend=True, linewidth=self.linewidth)
                    else:
                ...
                if batches_per_epoch and epoch and log_interval:
                    ...
                    for line_position in range(0, int(xlimits[1]), batches_per_epoch//log_interval):
            ...
            print("Column (", c, ") statistics: ", min(val_df[c]), " -- ", max(val_df[c]))
